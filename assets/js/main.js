...
(function () {
   emailjs.init({
      publicKey: "RosD10Gq1YFHl5oIc",
   });
})();

johnContactForm = document.getElementById("john-contact-form");
johnContactFormAlert = document.querySelector(".contact-form-alert");

johnContactForm.addEventListener('submit', function (event) {
   event.preventDefault();
   // these IDs from the previous steps
   emailjs.sendForm('service_wv7wr02', 'template_1x1n1g7', '#john-contact-form')
      .then(() => {
         //console.log('SUCCESS!');
         johnContactFormAlert.innerHTML = "<span>Your message sent successfully!</span><i class='ri-checkbox-circle-fill'></i>";
         johnContactForm.reset();
         setTimeout(() => {
            johnContactFormAlert.innerHTML = "";
         }, 5000)
      }, (error) => {
         johnContactFormAlert.innerHTML = "<span>Message not sent</span><i class='ri-error-warning-fill'></i>";
         johnContactFormAlert.title = error;
      });
});
/* =====================================================
...
window.addEventListener("scroll",()=>{
   ...
   johnHeader.classList.toggle("shrink", window.scrollY>0);
});
/* =====================================================
...
const cursorHoverlinks = document.querySelectorAll("body a, .theme-btn, .john-main-btn, .portfolio-card, .swiper-button-next, .swiper-button-prev, .swiper-pagination-bullet, .service-card, .contact-social-links li, .contact-form .submit-btn, .menu-show-btn, .menu-hide-btn");
...
themeBtn.addEventListener("click", ()=>{
   ...
   localStorage.setItem("john-saved-icon", getCurrentIcon());
   localStorage.setItem("john-saved-theme", getCurrentTheme());   
});

// Get saved theme icon and theme on document loaded.

const savedIcon = localStorage.getItem("john-saved-icon");
const savedTheme = localStorage.getItem("john-saved-theme");
...
ScrollReveal().reveal('.about-info, .john-footer .john-logo', { delay: 300, origin: 'bottom' });
ScrollReveal().reveal('.pro-card, .about-buttons, .john-main-btn, .resume-tabs .tab-btn, .portfolio-tabs .tab-btn', { delay: 500, origin: 'right', interval: 200 });
